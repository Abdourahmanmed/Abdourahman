...
export function ContactSection() {
  return (
    <AnimatedSection id="contact" className="py-18 md:py-24">
      <div className="relative overflow-hidden rounded-3xl border border-border/70 bg-card/80 p-6 shadow-[0_22px_70px_-35px_rgb(0,0,0,0.45)] backdrop-blur-md md:p-10">
        ...
        <div className="grid items-center gap-8 lg:grid-cols-[1.2fr_1fr] lg:gap-10">
          <div>
            ...
            <h2 className="mt-4 max-w-2xl text-3xl font-semibold tracking-tight text-balance text-foreground md:text-4xl lg:text-5xl">
              Construisons ensemble une expérience digitale qui inspire confiance et convertit.
            </h2>

            <p className="mt-4 max-w-xl text-base leading-relaxed text-muted-foreground md:text-lg">
              Vous avez un projet web, SaaS ou mobile ambitieux ? Parlons de vos objectifs et transformons-les
              en un produit premium, performant et prêt à scaler.
            </p>
            ...
            <div className="mt-7 flex flex-wrap gap-3">
              {contactLinks.map((link, index) => {
                ...
                return (
                  ...
                  >
                    <Button asChild size="lg" variant={link.variant} className="h-10 rounded-xl px-4 text-sm">
                      <a href={link.href} target="_blank" rel="noreferrer" aria-label={`Me contacter via ${link.label}`}>
          ...
          >
            <p className="text-xs font-semibold tracking-[0.12em] text-muted-foreground uppercase">Point de contact direct</p>
            <p className="mt-2 text-xl font-semibold text-foreground md:text-2xl">Réponse sous 24h</p>
            <p className="mt-3 text-sm leading-relaxed text-muted-foreground">
              Je sélectionne peu de collaborations pour garantir un haut niveau d&apos;exécution. Si votre vision est
              claire et ambitieuse, je serais ravi d&apos;en discuter.
            </p>
            ...
            <div className="mt-6 space-y-3">
              {contactLinks.map((link) => (
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="group flex items-start justify-between rounded-xl border border-border/70 bg-card/70 px-4 py-3 transition-colors hover:border-primary/40 hover:bg-primary/5"
                >
                  <div>
